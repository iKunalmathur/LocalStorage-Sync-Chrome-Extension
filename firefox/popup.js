// Use browser API for Firefox (compatible with both browser and chrome)
...
async function getDataFromSource() {
    ...
    const sourceTabs = await API.tabs.query({ url: sourcePattern });
...
async function setDataOnTarget(data) {
    ...
    const baseTarget = new URL(targetUrl).origin; // Get just the origin (e.g., http://localhost:3000)
    
    // Query all tabs and filter by matching the base URL
    const allTabs = await API.tabs.query({});
    let targetTabs = allTabs.filter(tab => tab.url && tab.url.startsWith(baseTarget));

    if (!targetTabs.length) {
    const tab = await API.tabs.create({ url: targetUrl });
      targetTabs = [tab];
...
async function loadSettings() {
    const result = await API.storage.sync.get(['sourceHost', 'targetHost', 'storageKey']);
...
saveBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    await API.storage.sync.set({ sourceHost, targetHost, storageKey });
